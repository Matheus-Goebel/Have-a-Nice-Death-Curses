...
function copyImage(img) {
    ...
    if (!copiedImagesMap[src]) {            
        ...
        if (existingBoxWithSpace) {
            existingBoxWithSpace.querySelector('.copied-images').appendChild(copiedImg);
        } else {
            createNewCopiedBox().appendChild(copiedImg);
        }
    }
}    

function removeCopiedImage(src) {
    if (copiedImagesMap[src]) {
